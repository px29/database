...
public class JoinAttributesExtraction  implements ExpressionVisitor  {
	...
	public ArrayList<SchemaPair> getLeft(){
		return leftRelaionSchema;
	}
	
	public ArrayList<SchemaPair> getRight(){
		return rightRelationSchema;
	}
	@Override
	...
	public void visit(EqualsTo arg0) {
		...
		SchemaPair leftSchema= new SchemaPair(ltable, ((Column) left).getColumnName());
		SchemaPair rightSchema= new SchemaPair(rtable, ((Column) right).getColumnName());
		if (index1 > index2) {
			leftRelaionSchema.add(rightSchema);
			rightRelationSchema.add(leftSchema);
		} else {
			leftRelaionSchema.add(leftSchema);
			rightRelationSchema.add(rightSchema);
		}
	}
